      ...
      <Dialog open={!!selectedEvent} onOpenChange={() => setSelectedEvent(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
          {selectedEvent && (() => {
            ...
            const visibleHighlights = event.highlights.filter((highlight) => hasText(highlight));
            const hasDescription = hasText(event.description);
            const hasCapacity = hasText(event.capacity);
            const hasContact = hasText(event.contact);
            const hasPhone = hasText(event.phone);
            const hasDetails = hasText(event.details);
            const hasRequirements = hasText(event.requirements);
            const hasRegistration = hasText(event.registration);
